...
output$dateTable <- DT::renderDT({
  if(!is.null(loadedData$dates_first_diagnosis)){
    DT::datatable(loadedData$dates_first_diagnosis, escape=FALSE, selection = 'single', rownames = FALSE, options = list(scrollX = TRUE, scrollY = TRUE, pageLength = 5, lengthMenu = c(5, 10, 15, 20))
    )
  }
})
...
output$AddPatientDateUI <- renderUI({
  ...
  fluidPage(
    fluidRow(
      column(
        width = 8,
        selectInput(inputId="AddPatientIDDate",
                    label = "Select PATIENT_ID",
                    choices = c("", unique(patIDs[which(!is.na(patIDs))])),
                    selected = 1),
        dateInput(inputId="AddPatientDate",
...
observeEvent(input$ModalbuttonAddDate, {
  if(input$AddPatientIDDate==""){
    ...
    showNotification("Date cannot be empty.", type="error", duration = NULL)
  } else if(input$AddPatientIDDate %in% loadedData$dates_first_diagnosis$PATIENT_ID){
    showNotification("This PATIENT_ID has already a date. If you want to change the date, please use the 'Edit date' button.", type="error", duration = NULL)
  } else {
    loadedData$dates_first_diagnosis <- rbind(loadedData$dates_first_diagnosis, list(PATIENT_ID=input$AddPatientIDDate, DATE=as.character(input$AddPatientDate)))
    removeModal()
  }
})
...
observeEvent(input$ModalbuttonEditDate, {
  if(input$AddPatientIDDate==""){
    ...
    loadedData$dates_first_diagnosis[which(loadedData$dates_first_diagnosis$PATIENT_ID==input$EditPatientIDDate),"DATE"] <- as.character(input$EditPatientDate)
    removeModal()
  }
})
...
observeEvent(input$ModalbuttonDeleteDate, {
  ...
  loadedData$dates_first_diagnosis <- loadedData$dates_first_diagnosis[-entry,,drop = FALSE]
  removeModal()
})

# safe dates of first diagnosis
observeEvent(input$datesSave,{
  write.table(loadedData$dates_first_diagnosis, file.path("study", input$cancer_study_identifier, "dates_first_diagnosis.txt.temp"), append = FALSE, sep = "\t", row.names = F, col.names = TRUE, quote = FALSE)
  file.rename(file.path(study_dir, input$cancer_study_identifier, "dates_first_diagnosis.txt.temp"), file.path(study_dir, input$cancer_study_identifier, "dates_first_diagnosis.txt"))
  showNotification("Diagnosis dates saved successfully!", type="message", duration = 10)
})

# treatment timeline ####
# Data table output
output$treatmentTable <- DT::renderDT({
  hidenColsTreatment <-  which(colnames(loadedData$data_timeline_surgery) %in% c("EVENT_TYPE"))-1
  DT::datatable(loadedData$data_timeline_treatment,
            selection = "single",
            rownames = F, options = list(pageLength = 25, columnDefs = list(list(visible=FALSE, targets=c(hidenColsTreatment)))))
})
...
observe({loadedData$data_timeline_treatment <- treatment_addRow()})
...
observe({loadedData$data_timeline_treatment <- treatment_editRow()})
...
treatment_delRow <- callModule(
  ...
  data= reactive(loadedData$data_timeline_treatment),
  selected_row = reactive(input$treatmentTable_rows_selected))
observe({loadedData$data_timeline_treatment <- treatment_delRow()})
...
observe({loadedData$data_timeline_treatment <- treatment_delCol()})
...
treatment_addCol <- callModule(
  ...
  id="Treatment",
  data= reactive(loadedData$data_timeline_treatment))
observe({loadedData$data_timeline_treatment <- treatment_addCol()})

# save treatment data ####
callModule(
  module=save_timelineServer,
  id="Treatment",
  study_id = reactive(input$cancer_study_identifier),
  data= reactive(loadedData$data_timeline_treatment))

# surgery timeline ####
# Data table output
output$surgeryTable <- DT::renderDT({
  hidenColsSurgery <-  which(colnames(loadedData$data_timeline_surgery) %in% c("EVENT_TYPE", "STOP_DATE"))-1
  DT::datatable(loadedData$data_timeline_surgery,
            selection = "single",
            rownames = F, options = list(pageLength = 25, columnDefs = list(list(visible=FALSE, targets=c(hidenColsSurgery)))))
})
...
surgery_addRow <- callModule(
  ...
  dates_first_diagnosis = reactive(loadedData$dates_first_diagnosis),
  mode = "timepoint")
observe({loadedData$data_timeline_surgery <- surgery_addRow()})
...
surgery_editRow <- callModule(
  ...
  mode = "timepoint")
observe({loadedData$data_timeline_surgery <- surgery_editRow()})
...
surgery_delRow <- callModule(
  ...
  data= reactive(loadedData$data_timeline_surgery),
  selected_row = reactive(input$surgeryTable_rows_selected))
observe({loadedData$data_timeline_surgery <- surgery_delRow()})
...
surgery_delCol <- callModule(
  ...
  data= reactive(loadedData$data_timeline_surgery),
  exclude = c("PATIENT_ID", "START_DATE", "STOP_DATE", "EVENT_TYPE"))
observe({loadedData$data_timeline_surgery <- surgery_delCol()})
...
surgery_addCol <- callModule(
  ...
  id="Surgery",
  data= reactive(loadedData$data_timeline_surgery))
observe({loadedData$data_timeline_surgery <- surgery_addCol()})
...
output$statusTable <- DT::renderDT({
  hidenColsSurgery <-  which(colnames(loadedData$data_timeline_status) %in% c("EVENT_TYPE", "STOP_DATE"))-1
  DT::datatable(loadedData$data_timeline_status,
            selection = "single",
            rownames = F, options = list(pageLength = 25, columnDefs = list(list(visible=FALSE, targets=c(hidenColsSurgery)))))
})
...
status_addRow <- callModule(
  ...
  dates_first_diagnosis = reactive(loadedData$dates_first_diagnosis),
  mode = "timepoint")
observe({loadedData$data_timeline_status <- status_addRow()})
...
status_editRow <- callModule(
  ...
  mode = "timepoint")
observe({loadedData$data_timeline_status <- status_editRow()})
...
status_delRow <- callModule(
  ...
  data= reactive(loadedData$data_timeline_status),
  selected_row = reactive(input$statusTable_rows_selected))
observe({loadedData$data_timeline_status <- status_delRow()})
...
status_delCol <- callModule(
  ...
  data= reactive(loadedData$data_timeline_status),
  exclude = c("PATIENT_ID", "START_DATE", "STOP_DATE", "EVENT_TYPE"))
observe({loadedData$data_timeline_status <- status_delCol()})
...
status_addCol <- callModule(
  ...
  id="Status",
  data= reactive(loadedData$data_timeline_status))
observe({loadedData$data_timeline_status <- status_addCol()})
